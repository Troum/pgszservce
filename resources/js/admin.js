...
$(document).ready(function () {
    ...
    let video = $('#video'),
        ...
        filename = $('#filename'),
        videoSpeciality = $('#video-speciality'),
        upload = $('#upload'),
        codes = $('#codes'),
        generate = $('#generate'),
        email = $('#email'),
        ...
        registered = $('#registered').DataTable({
            'language': {
                "processing": "Подождите...",
                "search": "Поиск:",
                "lengthMenu": "Выводить _MENU_",
                ...
                "paginate": {
                    "first": "Первая",
                    "previous": "<i class=\"material-icons md-13\">\nkeyboard_arrow_left\n</i>",
                    "next": "<i class=\"material-icons md-13\">\nkeyboard_arrow_right\n</i>",
                    "last": "Последняя"
                },
                "aria": {
        ...
        fd = new FormData();
    videoSpeciality.awselect();
    videoSpeciality.on('change', function () {
        fd.append('videoSpeciality', $(this).val());
    });

    video.on('change', function (e) {
        fd.append('video', $(this).prop('files')[0]);
        filename.val($(this).prop('files')[0].name)
    });
    ...
    upload.on('click', function () {
        ...
        axios({
            ...
            $('#filename').val('');
            $('#video-speciality').val('Выберите из списка');
            $('#video').val('');
            console.log();
            ...
            switch(error.response.status) {
                case 422:
                    ...
                    {
                        toastr.error(error.response.data.errors[err][0]);
                    }
                    break;
                case 409:
        ...
        axios({
            ...
            switch(error.response.status) {
                ...
                case 409:
                    ...
                    break;
                case 400:
                    tostr.error(error.response.data.error);
            }
